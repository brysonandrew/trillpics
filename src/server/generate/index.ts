...
export const generate = async (
  ...
  const compositionOptions: SelectCompositionOptions =
    {
      serveUrl:SERVE_URL,
      id:"pic-series",
      inputProps:{...inputProps, base:'remotion'},
      logLevel: "verbose", 
      onBrowserLog:inputProps.onLog??console.log,
    };

    inputProps?.onLog?.("Selecting composition...")
  const composition =
    await selectComposition(
      compositionOptions
    );
    inputProps?.onLog?.("Selecting composition... DONE")
  ...
  const renderMediaOption: RenderMediaOptions =
    ...
    inputProps?.onLog?.("Rendering media...")

  const result: TGenerateOutput =
    await renderMedia(
      renderMediaOption
    );
    inputProps?.onLog?.("Rendering media... DONE")
